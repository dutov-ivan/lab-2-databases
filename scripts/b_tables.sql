...
CHECK (
    phone SIMILAR TO '(\+?[0-9]{7,15})' OR phone IS NULL
);

ALTER TABLE servicemen
ADD CONSTRAINT chk_email_format
CHECK (
    email SIMILAR TO '[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}' OR email IS NULL
);
...
ADD CONSTRAINT chk_msc_code CHECK (code > 0);
...
ADD CONSTRAINT chk_min_rank CHECK (rank_categories.min_rank >= 0);

ALTER TABLE rank_categories
ADD CONSTRAINT chk_max_rank CHECK (rank_categories.max_rank >= 0);

ALTER TABLE rank_categories
ADD CONSTRAINT chk_min_rank_leq_max CHECK (rank_categories.min_rank <= rank_categories.max_rank);
...
CREATE TABLE ranks_rank_attributes (
	rank_id BIGINT NOT NULL,
	rank_attribute_id BIGINT NOT NULL
);

ALTER TABLE ranks_rank_attributes
ADD CONSTRAINT pk_ranks_rank_attributes PRIMARY KEY (rank_id, rank_attribute_id);
...
ALTER TABLE rank_attribute_values
ADD CONSTRAINT chk_date_value_positive CHECK (
    value_date IS NULL OR value_date >= '1900-01-01'
);
...
CREATE TABLE  munition_type_attribute_values(
    attribute_id BIGINT NOT NULL,
